...
m = np.array([[8, 2, 22, 97, 38, 15, 0, 40, 0, 75, 4, 5, 7, 78, 52, 12, 50, 77, 91, 8],
...
for i in range(-16, 16):
    largest_in_row = get_largest(m.diagonal(i), top)
    if largest_in_row > top:
        top = largest_in_row

# Get the largest value for the DOWN and to the LEFT diagonal
flip_m = np.fliplr(m)
for i in range(-16, 16):
    largest_in_row = get_largest(flip_m.diagonal(i), top)
    if largest_in_row > top:
        top = largest_in_row
